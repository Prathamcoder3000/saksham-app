...
exports.register = async (req, res, next) => {
  ...
  // Security Check: Prevent open registration of Admin roles
  if (role === 'Admin') {
    return res.status(403).json({ 
      success: false, 
      message: 'Not authorized to register as Admin via public endpoint' 
    });
  }
  ...
  try {
    const user = await User.create({
      ...
      email,
      password,
      role: role || 'Caretaker' // Default fallback
    });
